...
with open("team.csv", "r", newline="", encoding="utf-8") as fichier:
    ...
    for i, line in enumerate(team_write):
        ...
        equipe,victoires,nuls,defaites,points=line[0],line[1],line[2],line[3],list()
...
with open("new_team.csv", "r", encoding="utf-8") as fichier: 
    lecteur = csv.DictReader(fichier) 
    for ligne in lecteur: 
        print(f"equipe: {ligne['equipe']}, victoires: {ligne['victoires']} , nuls: {ligne['nuls']} , defaites: {ligne['defaites']}, points: {ligne['points']}")     
